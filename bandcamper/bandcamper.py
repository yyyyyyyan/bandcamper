...
class Bandcamper:
    ...
    BANDCAMP_URL_REGEX = re.compile(
        _BANDCAMP_SUBDOMAIN_PATTERN + r"\.bandcamp\.com$", flags=re.IGNORECASE
    )
    ...
    def _get_url(self, name):
        ...
        if parsed_url.scheme and parsed_url.netloc:
            ...
            if (
                self.BANDCAMP_URL_REGEX.match(domain)
                or gethostbyname(domain) == self.CUSTOM_DOMAIN_IP
            ):
                scheme = self.params.get("preferred_scheme") or parsed_url.scheme
                url = urljoin(f"{scheme}://{domain.lower()}", parsed_url.path)
            else:
                self.screamer.error(f"{name} is not a valid Bandcamp URL")
        elif self.BANDCAMP_SUBDOMAIN_REGEX.match(name):
            url = f"https://{name.lower()}.bandcamp.com"
        else:
            self.screamer.error(f"{name} is not a valid Bandcamp subdomain")
        return url
